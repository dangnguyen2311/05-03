...
document.getElementById("login-form").addEventListener("submit", function(event) {
    ...
    if (validateLoginForm()) {
        // Nếu thông tin đăng nhập hợp lệ, có thể thực hiện các hành động tiếp theo ở đây, ví dụ: gửi yêu cầu đăng nhập đến server
        alert("Thông tin đăng nhập hợp lệ. Thực hiện các hành động tiếp theo ở đây.");
    }
});
...
document.getElementById("register-form").addEventListener("submit", function(event) {
    ...
    if (validateRegisterForm()) {
        ...
        alert("Thông tin đăng ký hợp lệ. Thực hiện các hành động tiếp theo ở đây.");
    }
});
